...
public class OrdemBeneficiamentoCustom {
	...
	public List<AnaliseQualidade> findAnalise(int ordem) {
		...
		String query = " SELECT NVL(a.ordem_producao, 0) ordemTecelagem, "
				...
				+ "		(select TO_DATE(j.data_termino, 'DD-MM-YYYY') from pcpb_040 j where j.ordem_producao = " + ordem
				+ "			and j.codigo_estagio = 29) dataOrdem  "
	...
	public List<AnaliseQualidade> findInfoLote(int ordem) {
		...
		String query = " SELECT m.codigo_informacao cod, "
				...
				+ "          NVL((SELECT o.valor_02 FROM basi_400 o "
				+ "                                  WHERE o.nivel = m.nivel_estrutura "
				+ "                                  AND o.grupo = m.grupo_estrutura "
				+ "                                  AND o.subgrupo = m.subgru_estrutura "
				+ "                                  AND o.tipo_informacao = m.tipo_informacao AND o.codigo_informacao = m.codigo_informacao), 0 ) min, "
				+ "          NVL((SELECT o.valor_03 FROM basi_400 o "
				+ "                                  WHERE o.nivel = m.nivel_estrutura "
				+ "                                  AND o.grupo = m.grupo_estrutura "
				+ "                                  AND o.subgrupo = m.subgru_estrutura "
				+ "                                  AND o.tipo_informacao = m.tipo_informacao AND o.codigo_informacao = m.codigo_informacao), 0 ) max, "
				...
				+ "          FROM pcpb_075 M, hdoc_001 n "
				+ "          WHERE n.tipo = m.tipo_informacao "
				+ "          AND n.tipo = 33 "
