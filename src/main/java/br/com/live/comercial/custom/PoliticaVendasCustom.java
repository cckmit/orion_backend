...
public class PoliticaVendasCustom {
	...
	public List<DivergenciasPoliticaVendas> findPedidosDivergencias(String opcoes) {
		...
		String query = " SELECT REGRAS.PEDIDO CODPEDIDO, "
				...
				+ "     WHERE f.cgc_9 = a.cli_ped_cgc_cli9 "
				+ "     AND f.cgc_4 = a.cli_ped_cgc_cli4 "
				+ "     AND f.cgc_2 = a.cli_ped_cgc_cli2 "
				+ "     AND f.tipo_cliente = e.tipo_cliente "
				+ "     AND g.cod_funcionario (+) = a.cod_funcionario "
				...
				+ "     AND NOT EXISTS (SELECT 1 FROM orion_com_260 c "
				+ "                        WHERE c.tipo = 8 "
				+ "                        AND c.desconto = a.desconto_item3 "
				+ "                        AND c.natureza_operacao = a.natop_pv_nat_oper "
				+ "                        AND c.forma_pagamento = a.cod_forma_pagto) "
				+ "     UNION "
				+ "     SELECT a.pedido_venda PEDIDO, "
				...
				+ "     AND a.cod_rep_cliente <> 162 "
				+ "		AND g.cod_funcionario (+) = a.cod_funcionario "
				+ "		AND NOT EXISTS (SELECT 1 FROM orion_com_260 e "
				+ "								 WHERE e.tipo = 10 "
				...
				+ "       e.descr_tipo_clien canal, "
				+ "       a.numero_controle NUMERO, "
				+ "       (select g.cod_funcionario || ' - ' || max(g.nome) from efic_050 g " +
				"                    where g.cod_funcionario = a.cod_funcionario " +
				"                    group by g.cod_funcionario) FUNCIONARIO, "
				...
				+ "          AND f.tipo_cliente = e.tipo_cliente "
				+ "          AND a.cod_rep_cliente <> 162 "
				+ "          GROUP BY a.pedido_venda, e.descr_tipo_clien, a.cod_ped_cliente, a.cli_ped_cgc_cli9, a.cli_ped_cgc_cli4, a.cli_ped_cgc_cli2, f.nome_cliente, a.cod_funcionario, "
				+ "           a.numero_controle) pedidos "
				+ "       WHERE not (TRUNC(pedidos.valor_saldo_pedi) - TRUNC(orion_fn_calcula_valor_liq_ped(pedidos.pedido))) between -5 and 5 ) REGRAS "
