...
public interface RegistroTarefaAtividadeProjetoRepository extends JpaRepository<RegistroTarefaAtividadeProjetoEntity, Long> {
    ...
    @Query("SELECT a FROM RegistroTarefaAtividadeProjetoEntity a WHERE a.idProjeto = :idProjeto ORDER BY a.dataInicio, a.horaInicio")
    List<RegistroTarefaAtividadeProjetoEntity> findAllByIdProjeto(@Param("idProjeto") Long idProjeto);

    @Query("SELECT a FROM RegistroTarefaAtividadeProjetoEntity a WHERE a.idProjeto = :idProjeto and a.idRegistroAtividade = :idRegistroAtividade ORDER BY a.dataInicio, a.horaInicio")
