...
public class TabelaPrecoService {
	...
	public RetornoValidacaoTabelaPreco validarTabPreco(List<ImportarTabPreco> itensTabela) {
		...
		for (ImportarTabPreco item : itensTabela) {
			...
			int existeRef = tabelaPrecoCustom.validarReferenciaCorCadastrada(item.referencia, corValidar);
	...
	public void insereTabelaPreco(int coluna, int mes, int sequencia, int nivelPreco, String nivel, String grupo, String subGrupo, String item, int serie, float valor) {
		tabelaPrecoCustom.insereTabelaPreco(coluna, mes, sequencia, nivelPreco, nivel, grupo, subGrupo, item, serie, valor);
	}
}
